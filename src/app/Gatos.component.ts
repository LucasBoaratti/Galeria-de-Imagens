...
@Component({
	...
	templateUrl: "./Gatos.component.html", //Caminho do arquivo HTML do componente
	styleUrls: ["./Gatos.component.css"], //Caminho para o CSS (pode adicionar mais, no futuro)
})

export class ComponenteGatos implements OnInit {
	...
	api_key: string = ""; //Adicione a chave da sua API aqui.
	...
	ngOnInit() { //Método que roda o componente assim que o componente inicia
		this.http.get<any[]>(`https://api.thecatapi.com/v1/images/search?limit=50&api_key=${this.api_key}`) //Realizando uma requisição GET para a API
